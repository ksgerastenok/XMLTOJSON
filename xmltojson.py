...
def parse(element):
    ...
    if((type(element) != type(tuple()))):
        for item in element.items():
            if((type(result.get(str("@" + xml.QName(item[0]).text).strip())) == type(None))):
                result[str("@" + xml.QName(item[0]).text).strip()] = list()
            if((type(item) != type(None))):
                result.get(str("@" + xml.QName(item[0]).text).strip()).append(parse(item))
    ...
    if((type(element) != type(tuple()))):
        for item in element.getchildren():
            if((type(result.get(str("#" + xml.QName(item.tag).text).strip())) == type(None))):
                result[str("#" + xml.QName(item.tag).text).strip()] = list()
            if((type(item) != type(None))):
                result.get(str("#" + xml.QName(item.tag).text).strip()).append(parse(item))
